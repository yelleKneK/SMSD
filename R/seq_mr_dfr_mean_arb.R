...
seq_mr_dfr_mean_arb <- function(data, A, c, gamma,
  ...
  if(pilot==FALSE){
    ...
    optimal_N <- as.integer(sqrt((A/c))*(sqrt(stats::var(data))+n^(-gamma)))

    Rk<-(1/n)*A*stats::var(data)+(c*n)

    if(verbose==FALSE){
      Outcome <- list(Risk=Rk, N=n, mean=mean(data),
