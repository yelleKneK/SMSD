...
#' Interval for normal random variables
#'
#'
...
#' and an indcator of if the criterion was satisified.
#'
...
#'
#' @export seq_fwd_norm_mean
#'
#' @examples
#' pilot_ss <- seq_fwd_norm_mean(alpha=0.1, d=1, gamma=1, pilot=TRUE)
#' SLS <- rnorm(pilot_ss)
#' seq_fwd_norm_mean(data=SLS, d=1, alpha=0.1, pilot=FALSE)
seq_fwd_norm_mean <- function(data, d, alpha, gamma,
                              pilot=FALSE, verbose=FALSE,
                              na.rm=TRUE){
  if(missing(d)){
  ...
  if(pilot==FALSE)
  {
    ...
    optimal_N <- as.integer(((qnorm(1-alpha/2)^2)*stats::var(data))/d^2)
