...
#' for distribution free random variables with known variance
#'
...
#'
#' @export seq_mr_dfr_mean
#'
#' @examples
#' pilot_ss <- seq_mr_dfr_mean(A=100, c=2, pilot=TRUE)
#' SLS <- rnorm(pilot_ss, mean=0, sd=1)
#' seq_mr_dfr_mean(data=SLS, A=100, c=2, lowerlim=2, pilot=FALSE)
seq_mr_dfr_mean <- function(data, A, c, lowerlim ,
  ...
  if(pilot==FALSE)
  {
    ...
    optimal_N <- as.integer(sqrt((A/c)*stats::var(data)))
